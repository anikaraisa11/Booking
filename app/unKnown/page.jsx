"use client";
import GlobalButton from "@/components/Global/GlobalButton";
import GlobalForm from "@/components/Global/GlobalForm";
import GlobalInput from "@/components/Global/GlobalInput";
import Header from "@/components/Header/Header";
import React from "react";
import { useForm } from "react-hook-form";

function UnKnown() {
  const { register, handleSubmit } = useForm();
  const onSubmit = async (data) => {};

  return (
    <div>
        <Header />
      <div className="customContainer">
        <div className="container">
          <div className="flex h-[70vh] py-5 items-center justify-center">
          <div className='w-full bg-white rounded-lg p-5 md:w-[50%]' style={{boxShadow: "rgba(99, 99, 99, 0.2) 0px 2px 8px 0px"}}>
          <h3 className="text-4xl text-center py-5 font-bold animation">Update  Password</h3>
          <GlobalForm onSubmit={onSubmit}>
              <GlobalInput name="old password" label="Enter Your Old Password" />
              <GlobalInput name="new password" label="Enter Your New Password" />
              <div className='text-center'>
                <GlobalButton text="Update"></GlobalButton>
              </div>
            </GlobalForm>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}

export default UnKnown;
